...
def get_args():
    ...
    parser.add_argument('--parsed', type=str, default=None, help='Path to a pre-parsed JSON file (list[list[str]] of lemmas per sentence). Skips spaCy.')
    parser.add_argument('--save-parsed', dest='save_parsed', type=str, default=None, help='If set, save the parsed lemma data to this JSON path for reuse.')
    parsed = parser.parse_args()
    return {
        "text_path": parsed.text_path,
        "spacy_pipeline": parsed.spacy_pipeline,
        "parsed": parsed.parsed,
        "save_parsed": parsed.save_parsed,
    }
...
def count_1ts(word_freqs, processed):
    ...
    Returns:
        dict[int, int]: For each amount of words known in [0, len(word_freqs)), the amount of 1T sentences
    """
    counts = Counter()
    for i in tqdm(range(len(word_freqs)), desc="Computing Data"):
        counts[i] = 0
        known = set(map(lambda x: x[0], word_freqs[:i]))
...
if __name__ == "__main__":
    ...
    if ARGS["parsed"]:
        ...
        if ARGS["save_parsed"]:
            with open(ARGS["save_parsed"], "w", encoding="utf-8") as f:
