...
Run a script with the path to a directory of `.txt` files and (optionally) a spaCy pipeline name:

```sh
python hill1t.py ./resources en_core_web_sm
```

For large corpora, spaCy parsing dominates runtime. Save the parsed lemmas once and reuse them on subsequent runs:

```sh
# First run: parse and cache
python hill1t.py ./resources en_core_web_sm --save-parsed parsed.txt

# Later runs: skip spaCy entirely
python hill1t.py --parsed parsed.txt
```
